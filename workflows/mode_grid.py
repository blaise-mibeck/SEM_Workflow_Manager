...
class ModeGridWorkflow(WorkflowBase):
    ...
    def _get_mode_from_metadata(self, metadata):
        ...
        # If we've reached here, fall back to the basic mode
        return basic_mode
    ...
    def create_grid(self, collection, layout=None, options=None):
        ...
        for i, (img_data, img) in enumerate(zip(images, pil_images)):
            ...
            # Handle ChemSEM images differently due to size/aspect ratio
            if img_data.get("mode") == "chemsem":
                ...
                # Create a white background for ChemSEM image
                cell_bg = Image.new('RGB', (cell_width, cell_height), color='white')
                
                # Paste the ChemSEM image onto the background
                cell_bg.paste(resized_img, (x_offset, y_offset))
                
                # Paste the combined image into the grid
                grid_img.paste(cell_bg, (x, y))
            else:
            ...
            if options.get("label_mode", True):
                ...
                # Add parameter info if it varies and option is enabled
                metadata_dict = img_data.get("metadata_dict", {})
                varying_parameters = collection.get("varying_parameters", {})
                
                # Add HV if it varies and option enabled
                if varying_parameters.get("high_voltage", False) and options.get("label_voltage", True):
                    high_voltage = metadata_dict.get("high_voltage_kV")
                    if high_voltage is not None:
                        mode_display += f" {abs(high_voltage)}kV"
